...
class MapScene extends Phaser.Scene {
  ...
  lastSafeX=785; lastSafeY=900; collisionHintUntil=0;
  ...
  isInsideUiPanel(x:number,y:number){
    // Die gemalten Legenden-/UI-Flächen der Karte sind keine Spielfläche.
    if(x<270 && y>840) return true;
    if(x>1260 && y<410) return true;
    if(x>1285 && y>800) return true;
    return false;
  }
  isWaterPixel(x:number,y:number){
    const px=Math.max(0,Math.min(1535,Math.round(x)));
    const py=Math.max(0,Math.min(1023,Math.round(y)));
    const c=this.textures.getPixel(px,py,'map');
    if(!c) return false;
    // Wasser ist auf der Zeichenhafen-Karte deutlich blau/türkis.
    // Wege, Brücken, Erde, Felsen und Gebäude bleiben betretbar bzw. werden später genauer gesperrt.
    const isBlueWater = c.blue > 55 && c.blue > c.red + 22 && c.blue >= c.green - 8;
    const isVeryDarkSea = c.blue > 35 && c.green > 25 && c.red < 35 && c.blue >= c.green;
    return isBlueWater || isVeryDarkSea;
  }
  isBlockedPosition(x:number,y:number){
    if(this.isInsideUiPanel(x,y)) return true;
    // Wir prüfen mehrere Punkte an den Füßen, damit die Figur nicht halb ins Wasser rutschen kann.
    const points = [
      [x, y+28], [x-12, y+22], [x+12, y+22], [x, y+12]
    ];
    return points.some(([px,py])=>this.isWaterPixel(px,py));
  }
  ...
  update(_t:number,dt:number){
    ...
    // Einfache Karten-Kollision: Wasser und gemalte UI-Flächen werden blockiert.
    if(this.isBlockedPosition(this.player.x,this.player.y)){
      body.setVelocity(0,0);
      this.player.setPosition(this.lastSafeX,this.lastSafeY);
      if(Date.now()>this.collisionHintUntil){
        this.collisionHintUntil=Date.now()+1400;
        this.questText.setText('Hier geht es nicht weiter. Der Weg muss über Stege, Brücken oder Land führen.');
